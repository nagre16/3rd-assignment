...
Interface::Interface(int x, int y, int sx, int sy, std::vector<vector<int>> elemek, GameMaster*g)
    : Widget(x,y,sx,sy)
{
    _clicked = false;
    elems = elemek;
...
void Interface::draw()
{
    ...
    for (int i = 0; i<7; i++) {
        gout << move_to(_x+i*30,_y) <<color(86,60,13) << line_to(_x+i*30,_y+_s_y);
    }
    for (int j = 0; j <6; j++) {
